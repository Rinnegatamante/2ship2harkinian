...
std::shared_ptr<IResource> ResourceLoader::LoadResource(std::shared_ptr<File> fileToLoad) {
    if (fileToLoad == nullptr) {
		printf("Failed to load resource: File not loaded\n");
        SPDLOG_ERROR("Failed to load resource: File not loaded");
        return nullptr;
    }

    if (fileToLoad->InitData == nullptr) {
		printf("Failed to load resource: ResourceInitData not loaded\n");
    ...
    if (factory == nullptr) {
		printf("LoadResource failed to find factory for the resource at path\n");
