...
static struct ShaderProgram* gfx_opengl_create_and_load_new_shader(uint64_t shader_id0, uint32_t shader_id1) {
    ...
    void *prog_bin = NULL;
    char fname[256];
    sprintf(fname, "ux0:data/2s2h/shader_cache/%08X/%016llX_%d.bin", shader_id1, shader_id0, SHADER_MAGIC);
    ...
    if (f) {
		...
		char dname[256];
		sprintf(dname, "ux0:data/2s2h/shader_cache/%08X", shader_id1);
		sceIoMkdir(dname, 0777);
	}
#endif
...
#if defined(USE_OPENGLES) || defined(__vita__) // workaround for no GL_DEPTH_CLAMP
